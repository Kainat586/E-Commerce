...
export class SellerOrdersService {
  ...
  async getSellerOrders(sellerId: number, page = 1, take = 20, status?: string, sort: 'asc' | 'desc' = 'desc') {
    ...
    const orders = await this.prisma.order.findMany({
      ...
      include: {
        product: {
          select: { id: true, name: true, imageUrl: true, price: true, storeId: true },
        },
        user: {
  ...
  async cancelOrder(orderId: number, sellerId: number) {
    return this.updateOrderStatus(orderId, sellerId, { status: 'CANCELLED' });
  }
}
